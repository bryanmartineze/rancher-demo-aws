...
data "aws_ami" "suse-sles-15-sp5" {
  ...
  filter {
    name   = "name"
    values = ["suse-sles-15-sp5*x86_64"]
  }
...
resource "null_resource" "install_rancher" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo sh -c 'curl -sfL https://get.rke2.io | INSTALL_RKE2_CHANNEL=v1.30 INSTALL_RKE2_TYPE=server sh -'",
      ...
      "sleep 50",  # Wait for Cert Manager installation
      ...
      "helm upgrade -i rancher rancher-stable/rancher --namespace cattle-system --set bootstrapPassword=rancherSecurePassword --set hostname=rancher.${aws_eip.rancher_eip.public_ip}.sslip.io",
      
      "sleep 120",  # Wait until rancher installation is complete
